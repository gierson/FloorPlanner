...
FloorPlanner to aplikacja webowa do **planowania układania podłóg** (paneli podłogowych, desek, płytek). Użytkownik rysuje ściany, tworząc pokoje, wstawia drzwi, a system **optymalizuje układ paneli** z uwzględnieniem dylatacji, docinków i ciągłości wzoru przez otwory drzwiowe.
...
| `js/engine/waste-calculator.js`      | Realne zapotrzebowanie desek dla pióro-wpustu: parowanie docinków (przeciwległe ściany), jodełka — deski A/B liczone osobno (konserwatywnie)   |
...
| Realny odpad pióro-wpust (parowanie docinków, A/B)     | OK     |
...
## Nowe funkcje (sesja 10.07.2026)

### 8. Realne obliczanie odpadów dla pióro-wpustu (`WasteCalculator`)

- **Pliki:** `js/engine/waste-calculator.js` (nowy), `js/engine/layout-engine.js`, `js/engine/optimizer.js`, `js/app.js`, `js/ui/summary-panel.js`, `index.html`, `tests/helpers.js`
- **Problem:** `panelsNeeded = ceil(sumaPól / poleDeski)` zakładał pełny recykling każdego ścinka — odpad zawsze < 1 deska na pokój (~1-2%), niezależnie od wzoru i offsetu. Tryb „economic" optymalizatora był przez to ślepy na odpad.
- **Model — proste panele:** deska przecięta raz daje dwa kawałki z fabrycznym zamkiem na przeciwnych końcach. Docinek z końca rzędu (fabryczny koniec „min") pasuje tylko na start rzędu przy PRZECIWLEGŁEJ ścianie (fabryczny koniec „max") — cięty koniec nie ma zamka. `WasteCalculator.compute` paruje kawałki min-end z max-end, gdy `lenA + lenB ≤ panelLength` (greedy: sort rosnąco/malejąco, max liczba par). Kawałki ripowane wzdłuż, cięte z obu końców lub nieprostokątne = własna deska, ścinek to odpad. Kawałki z tej samej komórki siatki (deska przez otwór drzwiowy) grupowane po `row:col` = jedna fizyczna deska.
- **Model — jodełka (wariant konserwatywny):** panele +45° (deska A) i −45° (deska B) to lustrzane, osobne produkty — liczone oddzielnie (`panelsNeededA`/`panelsNeededB`), ścinki ukośne nigdy nie są reużywane: każda komórka = 1 kupiona deska swojego typu.
- **Nowe pola stats:** `panelsNeededA`, `panelsNeededB` (null dla prostych), `reusedPairs`; `panelsNeeded`/`wasteArea`/`wastePercent` liczone z realnego zakupu. Agregacja pokojów wyciągnięta do `WasteCalculator.aggregate(statsList)` — używana przez `optimizer.optimizeAll` i `app.js` (layout:adjust).
- **UI:** karta „Panele potrzebne" pokazuje deski do kupienia (dla jodełki: `n (A: x + B: y)`), wcześniej pokazywała liczbę kawałków w układzie.
- **Efekt (pokój 4×3 m):** proste 1380×193 stagger third → 48 desek, odpad 6,1% (10 par reuse); jodełka 625×125 → 178 desek (A:100 + B:78), odpad 13,7%. Zgodne z praktyką (jodełka realnie 10–15%).
- **Testy:** `tests/engine/waste-calculator.test.js` — 21 testów: parowanie (ta sama strona ≠ para, przeciwległe = para, limit długości, oba końce cięte, rip, direction 90), grupowanie po komórce, jodełka A/B, aggregate, integracja przez `generateLayout` i `optimizeAll`.
